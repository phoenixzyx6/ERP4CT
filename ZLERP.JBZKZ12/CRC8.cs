...
namespace ZLERP.JBZKZ12
{
    public class CRC8 : ICRC
    {
        ...
        /// <summary>
        /// 8位CRC校验 产生校验码 需要被校验码和校验码
        /// </summary>
        /// <param name="OldCRC">旧的校验码</param>
        /// <param name="bval">被校验码</param>
        /// <returns> 产生校验码</returns>
        public byte ComputeCrc(byte oldCrc, byte bval)
        {
            return CRC8_Table[oldCrc ^ bval];
        }

        /// <summary>
        /// 8位CRC校验 产生校验码 只要被校验码
        /// </summary>
        /// <param name="bval"></param>
        /// <returns>返回校验码</returns>
        public long GetCrc(int bval)
        {
            crc = CrcSeed;
            crc = CRC8_Table[crc ^ (bval & 0XFF)];
            crc ^= CrcSeed;
            return crc;
        }
        ...
        /// <returns>成功:返回校验码；失败：-1表示参数错误</returns>
        public long GetCrc(string str)
        {
            if (String.IsNullOrEmpty(str))
                return -1;
            return GetCrc(Encoding.Unicode.GetBytes(str));
        }

        /// <summary>
        /// 8位CRC校验 产生校验码 只要被校验的字节数组
        /// </summary>
        ...
        /// <summary>
        /// 8 位 CRC 校验 产生校验码 要被校验的字节数组、起始结果位置和字节长
        /// </summary>
        /// <param name="buffer"></param>
        /// <param name="offset"></param>
        /// <param name="count"></param>
        /// <returns>成功:返回校验码；失败：-1表示参数错误</returns>
        public long GetCrc(byte[] buffer, int offset, int count)
        {
    ...
    {
        #region CRC 16 位校验表
        readonly ushort CrcSeed = 0xFFFF;
        /// <summary>
        /// 16 位校验表 Upper 表
        /// </summary>
    ...
    public class CRC32 : ICRC
    {
	    ...
	    /// <summary>
	    /// 32位 CRC 校验 产生校验码 要被校验的字节数组、起始结果位置和字节长度
	    /// </summary>
	    /// <param name="buffer"></param>
	    /// <param name="offset"></param>
	    /// <param name="count"></param>
