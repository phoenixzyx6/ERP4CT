        ...
        $(document).ready(function () {
            ...
            $("table[aliases]").hover(
  		        function () {
  		            ...
  		            $("#ContentInfo").html($(this).attr("aliases"))
			        .addClass("contentInfo")
			        .show()
			        .offset({ top: pos.top, left: pos.left - $("#ContentInfo").width() });
  		        },
  		        function () {
            ...
            //方式一：页面传值
            var recHisID = '<% =Request.QueryString["hisid"]%>';
            ...
            $.post(
                ...
                function (resp) {
                    if (resp.Result && resp.Data) {
                        taskid = resp.Data.ObjectID;
                        $.each($.parseJSON(resp.Data.ReportDataContent), function (i, n) {
                            if ($("[name=" + n.name + "]", "form").is("input:radio") || $("[name=" + n.name + "]", "form").is("input:checkbox")) {
                                $("[name=" + n.name + "]", "form").each(function () {
                                    if ($(this).val() == n.value | $(this).val() == n.value + '') {
                                ...
                                if (typeof (n.value == "string" && n.value.indexOf("Date(") > 0)) {
                                    $("[name=" + n.name + "]", "form").val(n.value);
                                } else {
                                    $("[name=" + n.name + "]", "form").val(n.value);
                                }
                            }
...
<body>
    ...
    <table width="670" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td>
                <form id="StuffForm" style="border: none;">
