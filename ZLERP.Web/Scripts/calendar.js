...
function loadcalendar() {
	...
	for(var k = 2020; k >= 2013; k--) {
		s += k != 2020 && k % 10 == 0 ? '</div><div class="col">' : '';
		s += '<a href="###" onclick="refreshcalendar(' + k + ', mm);$G(\'calendar_year\').style.display=\'none\'"><span' + (today.getFullYear() == k ? ' class="calendar_today"' : '') + ' id="calendar_year_' + k + '">' + k + '</span></a><br />';
	}
	s += '</div></div>';
	s += '<div id="calendar_month" onclick="doane(event)" style="display: none;z-index:100001;">';
	for(var k = 1; k <= 12; k++) {
		s += '<a href="###" onclick="refreshcalendar(yy, ' + (k - 1) + ');$G(\'calendar_month\').style.display=\'none\'"><span' + (today.getMonth()+1 == k ? ' class="calendar_today"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' 月</span></a><br />';
	}
	s += '</div>';
	...
	div.innerHTML = s;
	$G('append_parent').appendChild(div);
	document.onclick = function(event) {
		$G('calendar').style.display = 'none';
		$G('calendar_year').style.display = 'none';
		$G('calendar_month').style.display = 'none';
		if(BROWSER.ie && BROWSER.ie < 7) {
			$G('calendariframe').style.display = 'none';
			$G('calendariframe_year').style.display = 'none';
			$G('calendariframe_month').style.display = 'none';
		}
	};
	$G('calendar').onclick = function(event) {
		doane(event);
		$G('calendar_year').style.display = 'none';
		$G('calendar_month').style.display = 'none';
		if(BROWSER.ie && BROWSER.ie < 7) {
...
function settime(d) {
	...
	controlid.value = yy + "-" + zerofill(mm + 1) + "-" + zerofill(d) + (addtime ? ' ' + zerofill($G('hour').value) + ':' + zerofill($G('minute').value) : '');
}
...
function showcalendar(event, controlid1, addtime1, startdate1, enddate1) {
	...
	$G('calendar').style.left = p['left']+'px';
	$G('calendar').style.top	= (p['top'] + 20)+'px';
	doane(event);
	refreshcalendar(currday.getFullYear(), currday.getMonth());
	if(lastcheckedyear != false) {
		$G('calendar_year_' + lastcheckedyear).className = 'calendar_default';
		$G('calendar_year_' + today.getFullYear()).className = 'calendar_today';
	}
	if(lastcheckedmonth != false) {
		$G('calendar_month_' + lastcheckedmonth).className = 'calendar_default';
		$G('calendar_month_' + (today.getMonth() + 1)).className = 'calendar_today';
	}
	$G('calendar_year_' + currday.getFullYear()).className = 'calendar_checked';
	$G('calendar_month_' + (currday.getMonth() + 1)).className = 'calendar_checked';
	$G('hourminute').style.display = addtime ? '' : 'none';
	lastcheckedyear = currday.getFullYear();
	lastcheckedmonth = currday.getMonth() + 1;
	if(BROWSER.ie && BROWSER.ie < 7) {
		$G('calendariframe').style.top = $G('calendar').style.top;
		$G('calendariframe').style.left = $G('calendar').style.left;
		$G('calendariframe').style.width = $G('calendar').offsetWidth;
		$G('calendariframe').style.height = $G('calendar').offsetHeight;
		$G('calendariframe').style.display = 'block';
	}
}
...
function refreshcalendar(y, m) {
	...
	mm = x.getMonth();
	$G("year").innerHTML = yy;
	$G("month").innerHTML = mm + 1 > 9  ? (mm + 1) : '0' + (mm + 1);
	...
	if(addtime) {
		$G('hour').value = zerofill(hh);
		$G('minute').value = zerofill(ii);
	}
}

function showdiv(id) {
	var p = fetchOffset($G(id));
	$G('calendar_' + id).style.left = p['left']+'px';
	$G('calendar_' + id).style.top = (p['top'] + 16)+'px';
	$G('calendar_' + id).style.display = 'block';
	if(BROWSER.ie && BROWSER.ie < 7) {
		$G('calendariframe_' + id).style.top = $G('calendar_' + id).style.top;
		$G('calendariframe_' + id).style.left = $G('calendar_' + id).style.left;
		$G('calendariframe_' + id).style.width = $G('calendar_' + id).offsetWidth;
		$G('calendariframe_' + id ).style.height = $G('calendar_' + id).offsetHeight;
		$G('calendariframe_' + id).style.display = 'block';
	}
}
