...
{
    public class AIR2ExamService : ServiceBase<AIR2Exam>
    {
        internal AIR2ExamService(IUnitOfWork uow)
            : base(uow)
        {
        }
        /*
        public override AIR2Exam Add(AIR2Exam entity)
        {
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
             ...
             .Query().Where(m => m.CommissionID == entity.CommissionID && entity.CommissionID != null && m.ExamineItemName.Contains("AIR2")).ToList();
                    if (list.Count>0)
                    {
                        logger.Error("该委托单下已经有该类型委托试验:");
                        throw new Exception("该委托单下已经有该类型委托试验");
                    }
                    AIR2Exam item = base.Add(entity);

                    CommissionItem commissionItem = new CommissionItem();
                    commissionItem.CommissionID = item.CommissionID;
                    commissionItem.ExamineItemName = "AIR2E";
        ...
        public override void Delete(AIR2Exam entity)
        {
        ...
        */
        /*2014-1-2 由XJM注释
        public override void Update(AIR2Exam entity, NameValueCollection form)
        {
            ...
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    List<CommissionItem> list = this.m_UnitOfWork.GetRepositoryBase<CommissionItem>()
         .Query().Where(m => m.CommissionID == entity.CommissionID && m.ExamineItemNameID != entity.ID && m.ExamineItemName.Contains("AIR2")).ToList();
                    if (list.Count > 0)
                    {
                        logger.Error("该委托单下已经有该类型委托试验:");
                        throw new Exception("该委托单下已经有该类型委托试验");
