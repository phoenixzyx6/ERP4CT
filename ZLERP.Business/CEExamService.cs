...
{
    public class CEExamService : ServiceBase<CEExam>
    {
        internal CEExamService(IUnitOfWork uow)
            : base(uow)
        {
        }
        /*
        public override CEExam Add(CEExam entity)
        {
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
             ...
             .Query().Where(m => m.CommissionID == entity.CommissionID && entity.CommissionID != null && m.ExamineItemName.Contains("CE")).ToList();
                    if (list.Count > 0)
                    {
                        logger.Error("该委托单下已经有该类型委托试验");
                        throw new Exception("该委托单下已经有该类型委托试验");
                    }
                    CEExam item = base.Add(entity);

                    //CommissionItem commissionItem = new CommissionItem();
                    //commissionItem.CommissionID = item.CommissionID;
                    //commissionItem.ExamineItemName = "CEE";
        ...
        public override void Delete(CEExam entity)
        {
        ...
        */
        /*2014-1-2 由XJM注释
        public override void Update(CEExam entity, NameValueCollection form)
        {
            ...
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    List<CommissionItem> list = this.m_UnitOfWork.GetRepositoryBase<CommissionItem>()
         .Query().Where(m => m.CommissionID == entity.CommissionID && m.ExamineItemNameID != entity.ID && m.ExamineItemName.Contains("CE")).ToList();
                    if (list.Count > 0)
                    {
                        logger.Error("该委托单下已经有该类型委托试验:");
                        throw new Exception("该委托单下已经有该类型委托试验");
