...
{
    public class FAExamService : ServiceBase<FAExam>
    {
        internal FAExamService(IUnitOfWork uow)
            : base(uow)
        {
        }
        /*
        public override FAExam Add(FAExam entity)
        {
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
             ...
             .Query().Where(m => m.CommissionID == entity.CommissionID && entity.CommissionID != null && m.ExamineItemName.Contains("FA")).ToList();
                    if (list.Count>0)
                    {
                        logger.Error("该委托单下已经有该类型委托试验:");
                        throw new Exception("该委托单下已经有该类型委托试验");
                    }
                    FAExam item = base.Add(entity);

                    CommissionItem commissionItem = new CommissionItem();
                    commissionItem.CommissionID = item.CommissionID;
                    commissionItem.ExamineItemName = "FAE";
        ...
        public override void Delete(FAExam entity)
        {
        ...
        */
        /*2014-1-2 由XJM注释
        public override void Update(FAExam entity, NameValueCollection form)
        {
            ...
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    List<CommissionItem> list = this.m_UnitOfWork.GetRepositoryBase<CommissionItem>()
         .Query().Where(m => m.CommissionID == entity.CommissionID && m.ExamineItemNameID != entity.ID && m.ExamineItemName.Contains("FA")).ToList();
                    if (list.Count > 0)
                    {
                        logger.Error("该委托单下已经有该类型委托试验:");
                        throw new Exception("该委托单下已经有该类型委托试验");
